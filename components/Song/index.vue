...
function findSong() {
  const time =
    Math.floor(Math.random() * (qusArr.arr[score.arr.length].second * 0.7)) + 1;
...
function ansShow() {
  ...
  while (ansList.arr.length < 3) {
    ...
    if (ansList.arr.filter((e) => e.id == nextList[A].SongID).length == 0) {
      ansList.arr.push({
        name: nextList[A].Artist,
        song: nextList[A].SongTitle,
        id: nextList[A].SongID,
      });
    }
  }
  ansList.arr.push({
...
<template>
  <div
    class="w-full h-full flex flex-col justify-center items-center bg-gray-200 p-4"
  >
    ...
    <h1 v-if="status !== 2" class="text-2xl font-bold text-gray-900">
      {{ title }}猜歌測驗
      {{ mode == "time" ? "時間" : mode == "normal" ? "一般" : "綜合" }}模式
    </h1>
    ...
    <div v-if="status !== 2" class="w-full lg:w-[800px] my-4">
      <div
        class="flex justify-between w-full items-center gap-2 text-sm text-gray-900"
      >
        <div
          class="flex rounded overflow-hidden divide divide-gray-100 shadow shadow-gray-300"
        >
          <button
            @click="changeFrom()"
      ...
      <div class="w-full flex justify-between items-center mt-4">
        <p v-if="mode !== 'normal'" class="text-sm text-gray-900">
          花費時間 {{ spendTime / 100 }}
        </p>
        <h1 v-show="ansList.arr.length > 0" class="text-sm">
    ...
    >
      <div class="relative">
        <div
          :class="status == 1 ? 'w-[90vw] lg:w-[800px]' : 'w-0'"
        ...
        <h1
          v-if="status == 1"
          class="text-base lg:text-xl text-gray-900 font-bold my-4 px-2 lg:px-4"
        >
        ...
        >
          <div v-show="score.arr.length !== qustime" class="text-center">
            ...
            <p v-if="mode !== 'normal'" class="text-sm text-white mt-4">
              花費時間 {{ spendTime / 100 }}
            </p>
          </div>
