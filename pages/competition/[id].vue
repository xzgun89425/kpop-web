...
const rules = reactive([
  ...
  },
  {
    ...
    rule: "抽10題，答錯不倒扣。",
    score:
      "第一次答對:10分<br/>第二次答對:7分<br/>第三次答對:4分<br/>第四次答對:1分<br/>每題得分加總，滿分100",
    top: "95分以上",
    master: "85分以上",
    gold: "75分以上",
    sliver: "60分以上",
  ...
  },
  {
    ...
    rule: "抽10題，答錯分數不倒扣。",
    score:
      "第一次答對:10分<br/>第二次答對:7分<br/>第三次答對:4分<br/>第四次答對:1分<br/>--------------<br/>2.5秒內答對:分數*1.5<br/>4秒內答對:分數*1<br/>4秒以上答對:分數*0.8<br/>(該題得分 * 該題秒數的加權指數) = 總得分",
    top: "135分以上",
    master: "100分以上",
    gold: "75分以上",
    sliver: "60分以上",
...
<template>
  <div
    v-if="step !== 3"
    class="w-full flex justify-center bg-gray-200 py-4 lg:py-8 px-4 lg:px-0"
  >
    <h1 class="text-2xl font-bold text-gray-900">{{ title }}猜歌測驗</h1>
  </div>
  <div
    v-show="step == 1"
    class="w-full flex justify-center items-start bg-gray-200 py-4 lg:py-8 px-4 lg:px-0"
  >
    <div
      class="w-full max-w-[800px] flex flex-col lg:flex-row items-center justify-start lg:justify-between gap-4"
    >
      <div
      ...
      >
        <svg
          ...
          class="w-12 h-12 fill-white mb-4"
          viewBox="0 0 512 512"
        >
  ...
  >
    <div
      class="bg-white w-full max-w-[800px] p-4 rounded shadow shadow-gray-400"
    >
      <p class="text-red-500 text-xl font-bold">
        注意：總題數{{ lists.arr.length }}題，{{ i.rule }}
      </p>
      <div class="w-full justify-between items-start flex py-8">
        <p class="text-base text-gray-800">
          評分機制：<br /><span v-html="i.score"></span><br />
        </p>
        <p>
          菁英：{{ i.top }}<br />
          大師：{{ i.master }}<br />
  ...
  <div
    v-if="step == 2"
    class="w-full flex justify-center bg-gray-200 py-8 px-4 lg:px-0"
  >
